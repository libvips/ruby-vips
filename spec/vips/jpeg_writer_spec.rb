...
describe VIPS::JPEGWriter do
  ...
  it "should detect icc headers and preserve the input image icc header" do
    ...
    jpeg.write @path
    im3 = VIPS::Image.read_jpeg @path
    jpeg2 = im3.to_jpeg
    jpeg2.icc?.should be_true
    jpeg.icc.should == jpeg2.icc
  end

  it "should remove the icc header" do
    im = simg 'icc.jpg'
    im2 = im.shrink 5, 5
    jpeg = im2.to_jpeg
  ...
  it "should remove the exif header" do
    im = simg 'huge.jpg'
    im2 = im.shrink 5, 5
    jpeg = im2.to_jpeg
  ...
  it "should allow attaching an external exif to the header" do
    ...
    jpeg2 = VIPS::Image.read_jpeg(@path).to_jpeg
  ...
  it "should accept options on creation from an image" do
    writer = @image.to_jpeg(:quality => 22)
